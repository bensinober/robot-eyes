...
pub const Window = struct {
    ...
    pub const Flag = enum {
        ...
        fn toEnum(wp: PropertyFlag, wf: anytype) Flag {
            const f = switch (@typeInfo(@TypeOf(wf))) {
                .Int, .ComptimeInt => @as(u32, @intCast(wf)),
                .Float, .ComptimeFloat => @as(u32, @intFromFloat(wf)),
                else => unreachable,
            };
            return switch (f) {
                0x00000000 => blk: {
    ...
    pub fn setTitle(self: *Self, title: []const u8) void {
        _ = c.Window_SetTitle(self.getCWindowName(), @ptrCast(title));
        self.name = title;
    }
    ...
    pub const Trackbar = struct {
        ...
        pub fn init(window_name: []const u8, trackbar_name: []const u8, max: i32) Trackbar {
            _ = c.Trackbar_Create(@ptrCast(window_name), @ptrCast(trackbar_name), max);
        ...
        pub fn getPos(self: Trackbar) i32 {
            return c.Trackbar_GetPos(@ptrCast(self.window_name), @ptrCast(self.trackbar_name));
        }
        ...
        pub fn setPos(self: *Trackbar, pos: i32) void {
            _ = c.Trackbar_SetPos(@ptrCast(self.window_name), @ptrCast(self.trackbar_name), pos);
        }
        ...
        pub fn setMin(self: *Trackbar, pos: i32) void {
            _ = c.Trackbar_SetMin(@ptrCast(self.window_name), @ptrCast(self.trackbar_name), pos);
        }
        ...
        pub fn setMax(self: *Trackbar, pos: i32) void {
            _ = c.Trackbar_SetMax(@ptrCast(self.window_name), @ptrCast(self.trackbar_name), pos);
        }
    };
};
